program ising_wolff
    ...
    integer, parameter :: N = 1e2                 ! Monte Carlo steps
    ...
    integer :: it, i, j, Si, n_add, ic, E, M, ip, jp, im, jm, in, jn
    real(8) :: r
    ...
    do it = 1,N
        ...
        do ic = 1,n_add
            ...
            if (S(ip,jn) == Si .AND. r < p .AND. .NOT. C(ip,jn)) then
                n_add = n_add+1
                s_add(1,n_add) = ip
                s_add(2,n_add) = jn
                C(ip,jn) = .TRUE.
            ...
            call random_number(r)
            if (S(im,jn) == Si .AND. r < p .AND. .NOT. C(im,jn)) then
                n_add = n_add+1
                s_add(1,n_add) = im
                s_add(2,n_add) = jn
                C(im,jn) = .TRUE.
            ...
            call random_number(r)
            if (S(in,jp) == Si .AND. r < p .AND. .NOT. C(in,jp)) then
                n_add = n_add+1
                s_add(1,n_add) = in
                s_add(2,n_add) = jp
                C(in,jp) = .TRUE.
            ...
            call random_number(r)
            if (S(in,jm) == Si .AND. r < p .AND. .NOT. C(in,jm)) then
                n_add = n_add+1
                s_add(1,n_add) = in
                s_add(2,n_add) = jm
                C(in,jm) = .TRUE.
